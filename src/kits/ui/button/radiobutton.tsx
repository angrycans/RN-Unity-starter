...
export default class RadioButton extends React.Component<PropsType, any> {
  static defaultProps = {
    disabled: false,
    activeStyle: {},
    loading: false,
  ...
  onPress = (...arg) => {
    if (this.props.delayPressDisable) {
      (this.props.onPress as any)(...arg);
    } else {
      if (this.props.onPress && this.state.delayPress) {
        this.setState({ delayPress: false, });
  ...
  render() {
    ...
    return (
      <TouchableHighlight
        ...
        style={[wrapperStyle, { borderRadius: 4 }, isChecked ? { backgroundColor: '#f35b91' } : {}]}
      ...
      >
        <View style={styles.container} onLayout={this.props.onLayout}>
          ...
          {icon
            ? <UI.ExIcon
              name={icon}
              style={[textStyle, styles.iconRawText, iconStyle]}
              onPress={this.onPress}
            />
            : null}
          <Text style={[textStyle, this.props.contentStyle, isChecked ? { color: 'white' } : {}]}>
            {this.props.children}
          </Text>
        </View>
      </TouchableHighlight>
